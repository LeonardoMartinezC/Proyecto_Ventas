...
tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["📁 Datos Generales", "📌 Análisis de Clientes", "📌 Análisis de Ventas", "📌 Envíos", "📌 Rentabilidad y Devoluciones"]
)
...
with tab3:
    ...
    st.write("### Los 5 Estados con mayor Ventas Brutas.")
    st.table(Data_ventas_estado.sort_values('Sales',ascending=False).head())
    primer_cuartil = Data_ventas_estado['Sales'].quantile(0.25)  # 25%
    mediana = Data_ventas_estado['Sales'].quantile(0.5)           # 50%
    tercer_cuartil = Data_ventas_estado['Sales'].quantile(0.75)
    lista_ventas = [str(primer_cuartil),str(mediana),str(tercer_cuartil),'Todos los Estados']
    

    # opciones = opciones.append('Todos los años')
    seleccion_ventas = st.selectbox('Selecciona una opción:', lista_ventas)
    Data_ventas_estado_dict = f.cambiar_abreviaturas_ventas(df_estados_abreviados,Data_ventas_estado)
    figura_ventas_estado = vsl.figura_mapa_ventas(Data_ventas_estado_dict,seleccion_ventas)
    st.plotly_chart(figura_ventas_estado,theme=None,use_container_width=True)
    st.write('Total de Ventas Brutas =',data_ordenes['Sales'].sum())
    ...
    st.write("### Los 5 Estados con mayor Ganancias Netas")
    st.table(data_ventas_n_estado.sort_values('Sales',ascending=False).head())
    primer_cuartil_netas = data_ventas_n_estado['Sales'].quantile(0.25)  # 25%
    mediana_netas = data_ventas_n_estado['Sales'].quantile(0.5)          # 50%
    tercer_cuartil_netas = data_ventas_n_estado['Sales'].quantile(0.75)  # 75%
    lista_ventas_netas = [str(primer_cuartil_netas),str(mediana_netas),str(tercer_cuartil_netas),'Todos los Estados']

    seleccion_ventas_netas = st.selectbox('Selecciona una opción:', lista_ventas_netas)

    Data_ventas_estado_dict_netas = f.cambiar_abreviaturas_ventas(df_estados_abreviados,data_ventas_n_estado)
    figura_ventas_netas = vsl.figura_mapa_ventas(Data_ventas_estado_dict_netas, seleccion_ventas_netas )
    st.plotly_chart(figura_ventas_netas,theme=None,use_container_width=True)
    st.write('Total de Ganancias Netas =',data_ordenes['Profit'].sum())
    ...
    # estado = vsl.Mapa_Estado()
    # st.plotly_chart(estado)
